...
import { addProduct } from '../api/vendor';
...
export default function AddProductScreen({ navigation }) {
  const [form, setForm] = useState({
    name: '', description: '', price: '', unit: UNITS[0],
    categorySlug: CATEGORIES[0], stockQuantity: '10', lowStockThreshold: '5',
  ...
  async function handleSubmit() {
    ...
    try {
      await addProduct({
        ...
        price, unit: form.unit, categorySlug: form.categorySlug,
        stockQuantity: parseInt(form.stockQuantity) || 10,
        lowStockThreshold: parseInt(form.lowStockThreshold) || 5,
        image: form.image.trim(), isHidden: form.isHidden,
      });
    ...
    <KeyboardAvoidingView style={s.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={s.inner}>
        ...
        <TouchableOpacity style={[s.submitBtn, loading && s.disabled]} onPress={handleSubmit} disabled={loading}>
          {loading ? <ActivityIndicator color="#000" /> : <Text style={s.submitText}>✅ Add Product</Text>}
        </TouchableOpacity>
      </ScrollView>
...
const s = StyleSheet.create({
  ...
  switchRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 20, marginBottom: 8 },
